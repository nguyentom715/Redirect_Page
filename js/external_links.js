...
(function ($, Drupal, drupalSettings) {
    ...
    attach: function (context, settings) {
      ...
      // use that.
      if (drupalSettings.redirect_page.redirect_page_safe_href_regexp) {
        safe_href_regexp = new RegExp(drupalSettings.redirect_page.redirect_page_safe_href_regexp);
      } else {
        safe_href_regexp = new RegExp("(\.gov|\.mil)");
      }

      // set up default redirect path if non is specified
      if (drupalSettings.redirect_page.redirect_node_path) {
        redirect_node_path = drupalSettings.redirect_page.redirect_node_path;
      } else {
        redirect_node_path = "/redirect";
      }

      redirect_new_window = drupalSettings.redirect_page.redirect_new_window;
      ...
      $("a:not(#external_link)", context).filter(hasAttr("href")).each(function () {
